...
# Mathematical formula for sum of interior angles
...
def angle_type_ques():
    ...
    # determines the angle type based on mathematical rules
    ...
    valid_answer = [angle_type, angle_type + " angle"]
    ...
    user_answer = input("What type of angle is it?  ").lower().strip() # converts users input into lowercase and removes spaces

    # exit code
    if user_answer == "xxx":
        return "quit", "User Quit"

    #checks the answer ( accepts angle type and angle type + the word "angle" at the end
    if user_answer in valid_answer:
        print("You're CORRECT!🥳")
        #returns true when the user enters the correct answer and stores it as a string for question history
        ...
        #returns false when the user enters the wrong answer and stores it as a string for question history
        return False, f"An Angle measures {angle_rules}° What type of angle is it? || Your Answer: ({user_answer}) Wrong❌❌❌)"

# generates a question with a random shape and missing angle
def missing_angle_gen():
    #  dictionary of shapes and the number of sides
    shape_list = {
        "Triangle": 3,
        "Quadrilateral": 4,
        "Pentagon": 5,
    ...
    while True:
        ...
        angles = []  # Stores the generated angles except the last one
        current_sum = 0 # tracks sum of angles so far

        for i in range(sides - 1):

            remaining_angles = sides - i - 1 # calculates how much remaining angles are left incl missing angle

            #Calculates the safe maximum possible angle and leaves at least 40° for every remaining angle
            max_possible = ( total_sum - current_sum) - (remaining_angles - 1 ) * 40

            #Generates an angle between 40 and the safe maximum angle
            angle = random.randint(40, min(160, max_possible))

            angles.append(angle) # adds generated angles into the list

            current_sum += angle #updates the current sum of angles

        missing_angle = total_sum - current_sum  # calculates the missing angle for the user to solve

        if 40 <= missing_angle < 180:  # checks if the missing angle is valid. It must be inbetween 40° and 180°
            break
...
# if users input is equal to yes
...
# if the user press <enter> it sets the mode
# to infinite mode for the correct rounds heading
#and sets the num_questions to 5
if num_questions == "infinite":
    mode = "infinite"
    num_questions = 5

# Keeps the quiz looping until the desired amount of questions requested by the user
# or exit code (xxx) is inputted
while num_questions_asked < num_questions:
    ...
    # Updates the questions right / questions wrong counter and the questions asked
    # for quiz statistics
    if correct:
        questions_right += 1
    else:
...
# if the user has answered more than 0 questions it prints out their statistics and accuracy
